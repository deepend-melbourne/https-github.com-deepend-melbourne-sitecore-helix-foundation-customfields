...
namespace Sitecore.Foundation.CustomFields.Pipelines
{
    public class ContentEditorInjectResources
    {
        private static readonly IList<string> _listScripts = new List<string>();
        private static readonly IList<string> _listStyles = new List<string>();
        ...
        static ContentEditorInjectResources()
        {
            var googleMapsApiKey = Settings.GetSetting("GoogleMapsApiKey");
            if (!string.IsNullOrWhiteSpace(googleMapsApiKey))
            {
                _listScripts.Add($"//maps.googleapis.com/maps/api/js?key={googleMapsApiKey}&amp;libraries=places");
            }
        }
